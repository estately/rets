...
class TestMetadataClass < MiniTest::Test
  ...
  def test_rets_class_build
    ...
    Rets::Metadata::TableFactory.stubs(:build).with(table_fragment, resource).returns(table)
    Rets::Metadata::RetsClass.stubs(:find_table_container).returns(table_container)
